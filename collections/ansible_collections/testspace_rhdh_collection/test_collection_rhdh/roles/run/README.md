Testspace_rhdh_collection.Test_collection_rhdh Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: testspace_rhdh_collection.test_collection_rhdh.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from testspace_rhdh_collection.test_collection_rhdh
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: testspace_rhdh_collection.test_collection_rhdh.run
      vars:
        run_x: 42
```
